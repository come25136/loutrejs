...
Extension-owned compiled valueはopaqueな信頼境界として保持する。Coreはそのshapeを解釈せず、clone、freeze、immutability検査を行わない。どの値がModel semanticsを構成し、どの実行対象のidentityを維持するかはExtensionだけが判断する。

Coreが所有するModel構造はbuild時点のimmutableなsnapshotである。Provider descriptor、dependency配列、conditional mapping、Lifecycle hookのdependency配列はbuild時に複製・固定する。Extension-owned compiled valueについては、ExtensionがModel semanticsに必要な範囲をsnapshotし、元DSL descriptorの変更から分離する契約を負う。RuntimeとGraphは同じsnapshotだけを参照する。handler factory、schema、user callback、Provider implementation class、`useValue`が指すservice instanceのような実行対象のidentityは維持する。
