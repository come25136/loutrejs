...
`request.headers` may also be an object union (`anyOf` / `oneOf` in Standard JSON Schema), so Content-Type-specific header requirements remain available to type inference and runtime validation. OpenAPI emits a safe merge as named Header Parameters and preserves cross-header correlations that standard fields cannot represent in the Operation Object `x-loutre-request-headers` extension.
