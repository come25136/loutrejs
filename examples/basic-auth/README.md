...
Protect `GET /profile` with HTTP Basic authentication. This example uses the `basicAuth()` Layer, injects a `UserRepository` into its factory, provides the authenticated user through a Context Key, short-circuits authentication failures, and returns a `WWW-Authenticate` challenge.
