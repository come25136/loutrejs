...
function renderCompactBanner(info: StartupBannerInfo): string {
  return [
    `Loutre ${info.version} (${info.application})`,
    `Server: ${info.server}`,
...
function renderRichBanner(
  ...
  const paint = (rgb: RGB, value: string) =>
    color
      ? `\u001B[38;2;${rgb[0]};${rgb[1]};${rgb[2]}m${value}\u001B[0m`
      : value
  const frame = (value: string) => paint(colors.frame, value)
  ...
  return [
    ...
    ...metadata.map(([label, value]) => {
      ...
      const styled = `${metadataIndent}${paint(colors.label, label)}${gap}${paint(colors.value, value)}`
      return row(plain, styled)
    }),
    row(''),
    `${frame('╰')}${border}${frame('╯')}`,
    '',
    `  ${paint(colors.value, '✓ Ready')} ${paint(colors.text, `in ${formatDuration(info.startupDurationMs)}`)}`,
  ].join('\n')
}
...
function renderLogo(color: boolean): {
  ...
  for (let row = 0; row < logoGlyphs[0].length; row += 1) {
    ...
    for (let letter = 0; letter < logoGlyphs.length; letter += 1) {
      ...
      styledParts.push(
        color
          ? `\u001B[38;2;${rgb[0]};${rgb[1]};${rgb[2]}m${padded}\u001B[0m`
          : padded,
      )
    }
