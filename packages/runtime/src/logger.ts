...
function formatConsoleLogRecord(
  ...
  const processLabel = runtimeProcess?.pid === undefined
    ? `[${prefix}]`
  ...
  const sourceLabel = source === undefined
    ? ''
    : `${paint(`[${source}]`, 33, colors)} `
  const formattedContext = formatLogContext(context)
  const contextSuffix = formattedContext === ''
    ? ''
    : ` ${paint(formattedContext, 90, colors)}`
...
function formatLogValue(value: unknown, ancestors = new WeakSet<object>()): string {
  try {
    ...
    try {
      ...
      return `{ ${Object.entries(value)
        .map(([key, item]) => `${formatLogKey(key)}=${formatLogValue(item, ancestors)}`)
        .join(', ')} }`
    } finally {
