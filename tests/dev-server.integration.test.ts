...
describe('Loutre Devtools control channel', () => {
  ...
  it('Graph Snapshotとactive baseをproject内.loutreへ永続化する', async () => {
    ...
    let client = await connectControl(server)
    let snapshotId = ''
    try {
      const created = await client.request<{
        id: string
        snapshot: DevtoolsGraphSnapshot
      }>('graph.snapshot.create', { name: 'before edit', setAsBase: true })
      snapshotId = created.id
      expect(created.snapshot).toEqual(snapshot())
      const state = JSON.parse(
        await readFile(
          join(projectRoot, '.loutre', 'devtools', 'state.json'),
          'utf8',
        ),
      ) as { activeBaseSnapshotId?: string }
      expect(state.activeBaseSnapshotId).toBe(snapshotId)
    } finally {
      client.close()
      await server.close()
    }
    ...
    server = await startDevServer({
      ...
      entry: 'src/app.ts',
      port: 0,
      loadGraph: async () => snapshot('ChangedLiveModule'),
    })
    client = await connectControl(server)
    try {
      const list = await client.request<{
        activeBaseSnapshotId?: string
        snapshots: Array<{ id: string; name?: string }>
      }>('graph.snapshots.list')
      expect(list.activeBaseSnapshotId).toBe(snapshotId)
      expect(list.snapshots).toContainEqual(
        expect.objectContaining({ id: snapshotId, name: 'before edit' }),
      )
      const stored = await client.request<{
        snapshot: DevtoolsGraphSnapshot
      }>('graph.snapshot.get', { snapshotId })
      expect(stored.snapshot).toEqual(snapshot())
      expect(await client.request('graph.base.get')).toEqual({
        base: expect.objectContaining({ id: snapshotId }),
      })
    } finally {
      client.close()
      await server.close()
  ...
  it('.loutre内のSnapshot書き込みではGraphをrebuildしない', async () => {
    let builds = 0
    const server = await startDevServer({
      projectRoot,
      entry: 'src/app.ts',
    ...
    try {
      await client.request('graph.snapshot.create', { setAsBase: true })
